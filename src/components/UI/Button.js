...
function Button({ children, href, onClick, variant }) {
  if (href) {
    return (
      <a
        className={`w-full ${variant === "primary" && "bg-gold hover:bg-yellow-800 hover:font-bold"} ${
        ...
        } py-5 text-xl rounded-lg hover:scale-110 transition-all duration-300 ease-in-out md:py-4`}
        href={href}
      >
        {children}
      </a>
